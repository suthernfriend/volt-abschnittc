...
const missingBallots = computed<string[]>(() => {
	...
	for (const ballotId of maleSeen.value) if (!maleTwice.value.includes(ballotId)) out.push(ballotId);
	for (const ballotId of femaleSeen.value) if (!femaleTwice.value.includes(ballotId)) out.push(ballotId);
...
const missmatchedBallots = computed<MissingBallotsType>(() => {
	...
	for (const ballotId of twiceBallot) {
		...
		if (ballots.length !== 2 || !voteEquals(ballots[0], ballots[1])) out[ballotId] = { counts: ballots };
	}
...
const ballotCounts = computed<BallotCountsType>(() => {
	...
	for (const count of election.value.counts.male) out[count.ballotId] = (out[count.ballotId] || 0) + 1;
	for (const count of election.value.counts.female) out[count.ballotId] = (out[count.ballotId] || 0) + 1;
	return out;
});
...
const maleSeen = computed<string[]>(() => {
	...
	for (const count of election.value.counts.male) {
		if (ballotCounts.value.hasOwnProperty(count.ballotId)) out.push(count.ballotId);
	}
	return [...new Set(out)];
});
...
const maleTwice = computed<string[]>(() => {
	const out: string[] = [];
	for (const count of election.value.counts.male) {
		if (ballotCounts.value.hasOwnProperty(count.ballotId) && ballotCounts.value[count.ballotId] > 1) out.push(count.ballotId);
	}
...
function addVote(gender: "male" | "female", vote: Vote) {
	election.value.counts[gender].push(vote);
	view.value = "start";
}
</script>
...
<template>
	<div class="container">
		...
		<div class="columns" v-if="view === 'input-md'">
			<div class="column">
				<vote-create @created="(vote) => addVote('male', vote)" gender="male" :candidates="election.candidates" />
			</div>
		</div>
		<div class="columns" v-if="view === 'input-wd'">
			<div class="column">
				<vote-create @created="(vote) => addVote('female', vote)" gender="female" :candidates="election.candidates" />
			</div>
		</div>
		<div class="columns" v-else-if="view === 'start'">
			...
			<div class="column">
				<div class="box">
					...
					<div v-if="Object.keys(missmatchedBallots).length === 0" class="block">Keine Fehler gefunden!</div>
